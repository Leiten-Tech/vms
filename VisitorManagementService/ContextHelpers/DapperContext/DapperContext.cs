...
namespace VisitorManagementMySQL.ContextHelper
{
    public class DapperContext : IDapperContext
    {
        ...
        public DapperContext(IConfiguration configuration)
        {
            ...
            connectionString = configuration.GetConnectionString("VisitorManagementCon");
        ...
        public async Task<SqlMapper.GridReader> ExecuteStoredProcedureAsync(
            string spName,
            object param = null
        )
        {
        ...
        public List<T> ExecuteQuery<T>(string Query, object param = null)
        {
            using (var connection = new MySqlConnection(connectionString))
            {
                var multi = connection
                    .Query<T>(Query, param, commandType: CommandType.Text)
                    .ToList();
                return multi;
            }
        }
        ...
        public async Task<List<T>> ExecuteQueryAsync<T>(string Query, object param = null)
        {
            using (var connection = new MySqlConnection(connectionString))
            {
                var multi = await connection.QueryAsync<T>(
                    Query,
                    param,
        ...
        public async Task<int> ExecuteNonReturnQueryAsync(string Query, object param = null)
        {
            using (var connection = new MySqlConnection(connectionString))
            {
                var multi = await connection.ExecuteAsync(
                    Query,
                    param,
                    commandType: CommandType.Text
                );
