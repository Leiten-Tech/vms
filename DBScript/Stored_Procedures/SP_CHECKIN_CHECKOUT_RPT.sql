...
BEGIN
    IF Type = 'CreateInitialize' THEN
        ...
        SELECT * FROM Plant p WHERE p.Status = 1 and p.Company_Id = CompanyId and p.Check_Token is not null and p.Check_Token != "";
...
WHERE 
(PlantIds IS NULL OR PlantIds = '' OR FIND_IN_SET(T.PlantId, PlantIds)) AND
